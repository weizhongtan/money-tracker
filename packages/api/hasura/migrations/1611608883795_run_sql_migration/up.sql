CREATE
OR REPLACE FUNCTION public.func_timeline(v_category_id uuid, v_group_by text) RETURNS SETOF table_amount_group LANGUAGE SQL STABLE AS $FUNCTION$ WITH raw_data AS (
  SELECT
    t.date,
    t.amount AS balance,
    CASE
