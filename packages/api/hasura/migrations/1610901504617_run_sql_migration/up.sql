CREATE OR REPLACE FUNCTION public.func_transactions_by_category_grouped (v_category_id uuid, v_group_by text)
  RETURNS SETOF table_transactions_by_category_grouped
  LANGUAGE sql
  STABLE
  AS $$ WITH raw_data AS (
  SELECT
  ...
  WHERE
    ...
    OR c.id = v_category_id -- if the transaction category does not match, try matching against the categories parent category
